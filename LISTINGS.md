...
| Glama **connector** | Aggregator | 🟡 Auto-ingested from the registry, but **Unhealthy** (last tested 2026-08-23 12:36) | Deploy anonymous discovery (§A of PUBLISHING.md), then claim via `/.well-known/glama.json` |
| Glama **servers directory** | Aggregator | ⬜ Not submitted (`/mcp/servers/rooquiz/rooquiz-mcp` 404) | Required for the score badge — §A2 of PUBLISHING.md |
...
because our authorization server only supports `authorization_code`. The unblock is anonymous
`initialize` / `tools/list` / `ping`, already implemented in rooquiz-payload.
